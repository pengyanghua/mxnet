...
class ZMQVan : public Van {
 ...
 protected:
  ...
  void Connect(const Node& node) override {
    ...
    if (it != senders_.end()) {
      zmq_close(it->second);  // reconnnect
    }
    // worker doesn't need to connect to the other workers.
    // @yhpeng: but necessary for server to scale parameters
    if ((node.role == my_node_.role) && (node.id != my_node_.id) &&
    		(my_node_.role == Node::WORKER)) {
      return;
    }
    //if ((node.role == my_node_.role) && (node.id != my_node_.id)) {
    //  return;
...
//    if(my_node_.id==9){
	//LOG(WARNING)<<"Node " <<my_node_.id<<" sending:"<<msg.DebugString();
  //  }
    CHECK_NE(id, Meta::kEmpty);
    auto it = senders_.find(id);
    if (it == senders_.end()) {
      LOG(WARNING) << "Node " << my_node_.id << ": there is no socket to node " << id;
      return -1;
    }
  ...
  int RecvMsg(Message* msg) override {
    if(my_node_.id==1){
	LOG(WARNING)<<"Scheduler Receive message:"<<msg->DebugString();
    }
    msg->data.clear();
    size_t recv_bytes = 0;
    for (int i = 0; ; ++i) {
      ...
      if (i == 0) {
        // identify
    	//LOG(INFO) << "GetNodeID: " << buf[0] << buf[1] << size;
	//LOG(INFO) << "NODE "<<my_node_.id<<" receive message!";
        msg->meta.sender = GetNodeID(buf, size);
        //LOG(INFO) << "NODE "<<my_node_.id<<" receive message from "<<msg->meta.sender;
