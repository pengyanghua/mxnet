...
//#include "../kvstore/kvstore_dist.h"
...
void GraphExecutor::Forward(bool is_train) {
  //std::cerr<<"$$$$$ Enter into GraphExecutor:Forward()\n";
...
void GraphExecutor::Backward(const std::vector<NDArray>& head_grads, bool is_train) {
  ...
  //std::cerr<<"Enter into GraphExecutor::Backward()\n";
...
void GraphExecutor::RunOps(bool is_train, size_t topo_start, size_t topo_end) {
  ...
  for (size_t nid = topo_start; nid < topo_end; ++nid) {
    ...
    opnode.exec->op_ctx.is_train = is_train;
      ...
      std::ifstream  myfile("node_id.txt");
...
//        std::getline(myfile,line);
  ...
  for (size_t nid = topo_start; nid < topo_end; ++nid) {
    ...
    if (monitor_callback_ == nullptr && seg_op.opr != nullptr && seg_op.topo_end <= topo_end) {
      ...
      profiling = true;
      bool record = true;
//      engine::ThreadedOpr* threaded_opr = engine::ThreadedOpr::CastFromBase(op);
//      std::cerr<<"Segments op is being pushed into engine:"<<threaded_opr.opr_name<<std::endl;
      Engine::Get()->Push(seg_op.opr, seg_op.ctx, 0, profiling, record,std::stoi(line));
    ...
    if (op_nodes_[nid].skip_exec_node) continue;
    opnode.exec->op_ctx.is_train = is_train;
    //std::cerr<<"Operator node is running:"<<opnode.opr_name<<std::endl;
	...
	std::getline(myfile,line);
      ...
      */
      //char* global_node_id = getenv("GLOBAL_NODE_ID");
      //std::cerr<<"Worker "<<line<<" in graphexecutor, pushed opr is:"<<opnode.opr_name<<" profiling is:"<<profiling<<"\n"<<std::flush;
      Engine::Get()->Push(opnode.cached_opr, opnode.ctx, 0, profiling,record,std::stoi(line));
    } else {
