...
Profiler::Profiler()
  ...
  myfile<<"Profiler init_time_:"<<this->init_time_<<"\n";
  
  myfile.open("operator.csv",std::ios_base::app);
  //myfile<<"Worker "<<ps::MyID()<<",Init_time_,"<<this->init_time_<<std::endl;
  myfile.close();  
...
void SetOprNodeID(OprExecStat* opr_stat, int node_id){
  if(!opr_stat){
    LOG(WARNING) << "SetNodeID: nullptr";
    return;
  }
  opr_stat->node_id = node_id;
}
