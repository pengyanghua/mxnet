...
void ThreadedEngine::Push(OprHandle op, Context exec_ctx, int priority, bool profiling,bool record,int node_id_) {
  ThreadedOpr* threaded_opr = ThreadedOpr::CastFromBase(op);
  OprBlock* opr_block = OprBlock::New();
  opr_block->opr = threaded_opr;
  if(threaded_opr->opr_name=="_backward_FullyConnected"){
    //std::cerr<<"Operator has been enter into engine. Name:"<<opr_block->opr->opr_name<<std::endl;
  }
  if(record&&false){
    std::ofstream myfile;
    char buf[80];
    ...
    wd = wd + "engine_output.txt";
    //std::cerr<<"Current direc:"<<wd<<std::endl;
    myfile.open(wd,std::ios_base::app);
    //myfile.open("~/elastic-mxnet/example/image-classification/results/engine_output.txt",std::ios_base::app);
    myfile<<"Operator has been enter into engine, name:"<<opr_block->opr->opr_name<<"\n";
    myfile.close();
  }
...
/*  std::ofstream myfile;
  ...
  myfile.open(wd,std::ios_base::app);
  //myfile.open("~/elastic-mxnet/example/image-classification/results/engine_output1.txt",std::ios_base::app);
  myfile<<"Operator has been enter into engine, name:"<<opr_name<<"\n";
  myfile.close();*/
  //std::cerr<<"In Engine PushAsync, pushed fn name:"<<opr_name<<" profiling:"<<profiling<<"\n"<<std::flush;
...
void ThreadedEngine::OnCompleteStatic(
  ...
  if (opr_block->profiling && threaded_opr->opr_name) {
    ...
    if(opr_block->node_id!=0){
      std::ofstream myfile;
/*      myfile.open("engine_output.txt",std::ios_base::app);
      myfile<<"Operator "<<opr_block->opr->opr_name<<" is actually completed, end time: "<<opr_block->opr_stat->opr_end_rel_micros<<"\n";*/
      //std::string file_name = "~/elastic-mxnet/example/image-classification/results/operator_"+std::to_string(opr_block->node_id)+".csv";
    ...
    //std::cerr<<"Buf of getcwd is:"<<buf<<std::endl;
      ...
      wd = std::string(buf) + std::string("/results/");
     // wd = wd + "engine_output1.txt";
      std::string file_name = wd+std::string("operator_")+std::to_string(opr_block->node_id)+".csv";
//      myfile.open(wd,std::ios_base::app)
      myfile.open(file_name,std::ios_base::app);
      std::string temp(opr_block->opr->opr_name);
      bool mark=true;
      char* temp_name = const_cast<char*>(temp.c_str());
      while(mark){
//	char* temp_name = const_cast<char*>(temp.c_str());
	char* position = std::strchr(temp_name,',');
      	if(position!=NULL){
		temp_name[position-temp_name]=' ';
      	} else{mark = false;}
      }
      myfile << opr_block->node_id <<',' << temp_name
	<<','<<opr_block->opr_stat->opr_start_rel_micros
	<<','<<opr_block->opr_stat->opr_end_rel_micros<<std::endl;
      myfile.close();
    }
/*
    struct op_record op_r;
    op_r.op_name = opr_block->opr->opr_name;
    op_r.start_time = opr_block->opr_stat->opr_start_rel_micros;
    op_r.end_time = opr_block->opr_stat->opr_end_rel_micros;
    record_operator.insert({count_op,op_r});
    count_op += 1;*/    
  }else if(threaded_opr->opr_name&&false){
      std::ofstream myfile;
      myfile.open("engine_output1.txt",std::ios_base::app);
      myfile<<"Operator "<<opr_block->opr->opr_name<<" is actually completed, end time: "<<opr_block->opr_stat->opr_end_rel_micros<<"\n";
      myfile.close();
