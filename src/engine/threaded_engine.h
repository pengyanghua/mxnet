...
class ThreadedEngine : public Engine {
 public:
  ...
  void Push(OprHandle op, Context exec_ctx, int priority = 0, bool profiling = false, bool record = false,int node_id = 0) override;
 ...
 protected:
  ...
  void ExecuteOprBlock(RunContext run_ctx, OprBlock *opr_block) {
    ...
    if (opr_block->profiling && threaded_opr->opr_name) {
      ...
      SetOprNodeID(opr_block->opr_stat,opr_block->node_id);
     // std::ofstream myfile;
     //myfile.open("engine_output.txt",std::ios_base::app);
      //myfile<<"Worker "<<opr_block->node_id<<" Operator "<<opr_block->opr->opr_name<<" is actually executed, start time: "<<opr_block->opr_stat->opr_start_rel_micros<<"\n";
      //myfile.close();     
      //int worker=ps::MyID(); 
    }
#endif
 ...
 private:
  ...
  void SetNodeID(int id=0) override{
	node_id = id;
  }
  ...
  std::unordered_map<int, struct op_record> record_operator;
  int count_op = 0;
  /*!
